...
'Ищем пациента'
WebUI.setText(findTestObject('Удаление пациента (Test case)/input_search_patient_Create'), GlobalVariable.Test_patient2_familia)

'Проверяем есть ли такой пациент'
WebUI.verifyElementText(findTestObject('Удаление пациента (Test case)/patient_Create'), GlobalVariable.Test_patient2_FIO_full)

'Открываем карточку пациента'
WebUI.click(findTestObject('Удаление пациента (Test case)/patient_Create'))

'Сверяем тот ли пациент'
WebUI.verifyTextPresent(GlobalVariable.Test_patient2_FIO_full, false)
